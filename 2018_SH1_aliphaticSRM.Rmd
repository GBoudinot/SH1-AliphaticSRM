---
title: "20180329_SH1_aliphaticsSRM"
output: 
  html_document:
    df: paged
    toc: true
...
outcome <- calib_data %>% 
  ...
  do({
    bind_rows(., 
        ratio_peaks(., "Ts C27 HO", c("Ts C27 HO", "Tm C27\\s\\sHO"), "C27Ts/Tm")
    )
  }) %>% 
  filter(!str_detect(file_id, "TSQ3465"), 
         str_detect(peak_name, "C27"), 
         str_detect(peak_name, "T[sm]"))
```
...
expand.grid(
  time = seq(0, 1000, by = 20),
  panel = c("A", "B", "C"),
  type = c("pizza", "jetpack", "turtle")
)
subset(final_data, peak_name== "C27Dia/Reg") %>%
tbl_df() %>% 
mutate(
  y = runif(n = length(time), min = 0, 
            max = 10*as.numeric(setNames(1:length(LETTERS), LETTERS)[panel]))
) %>% 
  # normalized data
  group_by(time, panel) %>% 
  mutate(y_norm = y/sum(y)) %>% 
  ungroup() %>% 
  ggplot() +
  aes(time, y, color = type) +
  geom_smooth(
    data = function(df) filter(df, panel %in% c("A", "C")),
    method = "loess", span = 0.5, se = FALSE
  ) +
  geom_point(
    data = function(df) filter(df, panel == "A")
  ) +
  geom_area(
    data = function(df) filter(df, panel == "B") %>% mutate(y = y_norm),
    mapping = aes(fill = type)
  ) + 
  geom_line(
    data = function(df) filter(df, panel == "C")
  ) +
...
final_data %>%
expand.grid(
  Depth = seq(90, 200, by = 20),
  panel = c("A", "B", "C"),
  type = c("C31S/S+R", "C32S/S+R", "C33S/S+R", "C34S/S+R", "C35S/S+R")
)  %>% tbl_df() %>% 
mutate(
  y = runif(n = length(depth), min = 0, 
            max = 10*as.numeric(setNames(1:length(LETTERS), LETTERS)[panel]))
)  %>% 
  # normalized data
  group_by(depth, peak_name) %>% 
...
outcome <- calib_data %>% 
  ...
  filter(#!str_detect(file_id, "TSQ3779"),  
         str_detect(peak_name, "Tm C27  HO")) 
