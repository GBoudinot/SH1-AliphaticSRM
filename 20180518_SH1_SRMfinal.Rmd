...
## Aromatic Data
```{r}
aromatic_final_data <- read.csv("Aromatic_SRM_SH1") %>% select(`depth`, `conc_rock.ug_g`, var = `peak_name`) 
...
extrapolated_timescale <-
  data_frame(
    depth = alldata_n_TOC$depth %>% unique()
  ) %>% 
  filter(!is.na(depth)) %>% 
  mutate(
...
alldata_w_time <- left_join(alldata_n_TOC, extrapolated_timescale, by = "depth")
```


```{r}
alldata_w_time %>% group_by(depth) %>%
  mutate(
  MAR =  30 * (2.5/10) 
)%>% ungroup() %>% 
  arrange(depth, var)

##black shales = 2.5 g/cm3
## bulk MAR (g/cm2/k.y.) = LSR (m/m.y.) x dry density (g/cm3)/10.
```
...
toTOC <- subset(alldata_w_time, var %in% c( "Chlorobactane", "Isorenieretane", "28, 30 C28 bisnor HO", "3MeHo_all", "all_PAH", "gamma HO", "2MeHo_all", "C30 aB HO")) %>%
  ggplot() +
...
ggplotly(sterane_area2)
```

```{r}
C26St_conc <-  subset(alldata_w_time, var %in% c("C26Regst", "C27Regst", "C28Regst", "C29Regst", "C30Regst")) %>%
  ggplot() +
  aes(x = ka, y = conc_rock.ug_g, color = var) +
  #facet_grid(~var, scales = "free") +
  geom_point() +
  geom_line() +
  coord_flip() +
...
```{r}
sterane_ratios <- subset(alldata_w_time, var %in% c( "C27St/allSt", "C28St/allSt", "C29St/allSt")) %>%
  ggplot() +
  aes(x = ka, y = value, color = var) +
  facet_grid(~var, scales = "free") +
  geom_point() +
...
triline <- subset(alldata_w_time, var %in% c("C19/tricyclics" ,  "C23/tricyclics" , "C24/tricyclics" )) %>%
  filter(ka < 250) %>%
  ggplot() +
  geom_point()+
  geom_line() +
...
holine <- subset(alldata_w_time, var %in% c("C29ab/allHoab","C30ab/allHoab", "C31ab/allHoab", "C32ab/allHoab", "C33ab/allHoab", "C34ab/allHoab", "C35ab/allHoab")) %>%
   ggplot() +
  #geom_area(mapping = aes(fill = var)) +
  aes(x = ka, y = value, color = var) +
  geom_point()+
...
### Stratification
```{r "", fig.width = 20, fig.height = 5}
HHIGI <- subset(alldata_w_time, var %in% c("d13c_org", "HHI", "Chlorobactane", "Isorenieretane", "C31_35_3MHI(%)", "GI")) %>%
  ggplot() +
  aes(x = ka, y = value, color = var) +
  geom_point() +
  #geom_line() +
...
```{r}
ThreeMeHo <- subset(alldata_w_time, var %in% c("28, 30 C28 bisnor HO", "3MeHo_all")) %>%
  ggplot() +
  aes(x = ka, y = conc_rock.ug_g, color = var) +
  geom_point() +
  #geom_smooth() +
...
```{r}
cross1 <- subset(alldata_w_time, value = conc_rock.ug_g) %>%
  ggplot() +
  aes(x = "28, 30 C28 bisnor HO", y = "3MeHo_all", color = var) +
  geom_point() +
  #geom_smooth() +
  facet_grid(~var, scales = "free") +
  #scale_x_reverse() +
...
### Terrestrial input 
```{r "", fig.width = 15, fig.height = 5}
terr <- subset(alldata_w_time, var %in% c("carb", "ngLCA/g rock", "C19/C19+23" , "OleananeIndex", "C29ab/allHoab", "C24tet/tricyclics")) %>%
  ggplot() +
  aes(x = ka, y = value, color = var) +
  geom_point() +
  #geom_line() +
...
### Steranes
```{r "steranes", fig.width = 25, fig.height = 10}
...
### Hopanes
```{r "tricyclics", fig.width = 20, fig.height = 10}
...
## Redox
```{r "Stratification", fig.width =25, fig.height = 10}
ggplotly(HHIGI)
```
...
### Dias
```{r "diasr", fig.width = 20, fig.height = 10}
ggplotly(diasr)
```
