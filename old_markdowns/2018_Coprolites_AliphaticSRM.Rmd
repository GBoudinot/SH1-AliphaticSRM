---
title: "2018_Coprolites_AliphaticSRM"
...
opts_chunk$set(dev=c("png", "pdf"), dev.args=list(pdf = list(encoding="WinAnsi", useDingbats=FALSE)),
  fig.keep="all", fig.path = file.path("plots", "coprolites_SRM")) ### CHANGE PLOT SUFFIX ###
```
...
all_data <- 
  ...
  list.files("data_coprolites", recursive = TRUE, full.names = TRUE, pattern = "\\.xls$") %>% 
...
```{r}
rock_info <- read_excel(file.path("metadata", "aliphaticSRM_coprolites.xlsx")) %>% 
  rename(rock.g = `rock`, tle = `TLE`, f1.g = `F1`, f2.g = `F2`, ref_amount_added.ug = `D4.ug` )%>% 
...
data_corrected <- 
  all_data %>%
  left_join(rock_info, by = "file_id") %>% 
  group_by(file_id) %>% 
  mutate(
    n_peaks = n(),
    ...
    conc_maltene.ug.g = amount.ug / f1.g
  )%>% ungroup() %>% 
...
```{r "overview_of locations", fig.width = 10, fig.height = 8}
  ggplot() +
  aes(x = conc_rock.ug_g, y = location, color = file_id) +
  geom_point() +
...
final_data %>% 
  ggplot() +
  aes(x = ratio, y = location, color = file_id) +
  geom_point() +
  facet_wrap(~peak_name, scales = "free") +
  scale_y_reverse() 
```
...
subset(final_data, peak_name=="D4 C29 ISTD") %>%  
  ggplot() +
  aes(x = file_id, y = amount.ug, color = file_id) + 
  geom_point(size = 3) +
  facet_wrap(~peak_name, scales = "free_y") +
    theme_bw() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
...
```{r}
subset(final_data, peak_name=="all_tricyclics") %>%  
  ggplot() +
  aes(x = file_id, y = conc_rock.ug_g, color = location) +
  geom_point(size = 3) +
  facet_wrap(~peak_name, scales = "free_y") +
    theme_bw() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
...
subset(final_data, peak_name=="C19/tricyclics") %>%  
  ggplot() +
  aes(x = file_id, y = ratio, color = location) +
  geom_point(size = 3) +
  facet_wrap(~peak_name, scales = "free_y") +
    theme_bw() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
