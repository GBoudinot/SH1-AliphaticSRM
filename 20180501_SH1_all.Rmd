...
## Aromatic Data
```{r}
aromatic_final_data <- read.csv("Aromatic_SRM_SH1") %>% select(`depth`, value = `conc_rock.ug_g`, `peak_name`) 

aliphatic_final_data <- final_data %>% select(`depth`, `conc_rock.ug_g`, value = `ratio`, `peak_name`)

SRM_all <- full_join(aromatic_final_data, aliphatic_final_data)
```
## n-Alkane Data
```{r}
alkane <- read_excel(file.path("metadata", "nalk_comp.xlsx")) %>% rename(depth = `meters`) 
...
all_data %>% filter(var %in% c("carb", "Osi", "d13c_org")) %>%
  ...
  bind_rows(
    SRM_all %>% tbl_df() %>% 
      filter(peak_name %in% c("C26St/allSt", "C27St/allSt", "C28St/allSt", "C29St/allSt", "C30St/allSt", "C30Me/allSt", "DinoSt/allSt")) %>% 
      select(depth, value , peak_name) %>% 
      mutate(var = "steranes")
  ) %>% 
  ...
  mutate(
    area = ifelse(var == "steranes", TRUE, FALSE),
    line = ifelse(var != "steranes", TRUE, FALSE),
    point = ifelse(var != "steranes", TRUE, FALSE)
  ) %>% 
  # format panels and colors properly
  mutate(
    latex_var = as_factor(var) %>% 
      fct_recode(
        "% carbonate" = "carb",
  ...
  geom_area(data = function(df) filter(df, area), 
            mapping = aes(color = NULL, fill = peak_name)) +
  scale_x_reverse() +
...
all_data %>% filter(var %in% c("carb", "Osi", "d13c_org")) %>%
  ...
  bind_rows(
    SRM_all %>% tbl_df() %>% 
      filter(var %in% c("C19/tricyclics" , "C20/tricyclics" , "C21/tricyclics" , "C22/tricyclics" , "C23/tricyclics" , "C24/tricyclics" , "C25/tricyclics" , "C26/tricyclics")) %>% 
      select(depth, value , var) %>% 
      mutate(vari = "Tricyclics")
  ) %>% 
  # make sure not to plot missing
  filter(!is.na(value)) %>% 
  # separate into points and lines and area
  mutate(
    area = ifelse(vari == "Tricyclics", TRUE, FALSE),
    line = ifelse(vari != "Tricyclics", TRUE, FALSE),
    point = ifelse(vari != "Tricyclics", TRUE, FALSE)
  ) %>% 
  # format panels and colors properly
  mutate(
    latex_var = as_factor(var) %>% 
...
### Stratification
```{r "Stratification", fig.width = 8, fig.height = 5}
HHIGI <- subset(SRM_all, peak_name %in% c( "C35/C35+C34", "HHI", "Chlorobactane", "Isorenieretane")) %>%
  ggplot() +
  aes(x = depth, y = ratio) +
  geom_point() +
  geom_line() +
